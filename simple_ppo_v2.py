...
from viz import RLTrainingVisualizer, setup_visualization, analyze_training_run
...
class HumanoidEnv:
    ...
    def step(self, action):
        ...
        info = {
            ...
            'center_of_mass': self.data.subtree_com[0].copy(),
            'upright_angle': np.abs(self.data.qpos[3:7]).max(),  # Quaternion deviation
            'energy_usage': np.sum(np.abs(self.data.ctrl * self.data.qvel[-len(self.data.ctrl):]))
        }
    ...
    def _calculate_reward(self):
        ...
        reward, _ = self._calculate_reward_with_breakdown()
        return reward
    
    def _calculate_reward_with_breakdown(self):
        """Calculate reward with detailed breakdown for visualization"""
        # Get current state
        ...
        return reward_dict['total'], reward_dict
...
class PPOAgent:
    """Proximal Policy Optimization Agent with RNN support and enhanced metrics"""
    ...
    def update(self, next_value, epochs=10, batch_size=64):
        """Update policy using PPO with enhanced metrics"""
        # Convert to tensors
        ...
        self.learning_stats = {
            ...
            'value_mean': torch.FloatTensor(self.values).mean().item(),
        ...
        for epoch in range(epochs):
            ...
            for start in range(0, dataset_size, batch_size):
                ...
                with torch.no_grad():
                    ...
                    clipped = ((ratios < 1 - self.clip_eps) | (ratios > 1 + self.clip_eps)).float()
                    clip_fractions.append(clipped.mean().item())
        ...
        self.learning_stats.update({
            ...
            'policy_std_mean': torch.exp(self.policy.log_std).mean().item()
        })
    ...
    def get_policy_stats(self):
        """Get current policy statistics for logging"""
        with torch.no_grad():
            policy_stats = {
                'action_std_mean': torch.exp(self.policy.log_std).mean().item(),
                'action_std_max': torch.exp(self.policy.log_std).max().item(),
                'action_std_min': torch.exp(self.policy.log_std).min().item(),
            }
            
            # Add weight statistics
            for name, param in self.policy.named_parameters():
                if param.requires_grad:
                    policy_stats[f'{name}_mean'] = param.data.mean().item()
                    policy_stats[f'{name}_std'] = param.data.std().item()
...
def train_humanoid_walker(xml_path, total_timesteps=100000, save_interval=10000, run_mode=None, enable_live_plots=False):
    """Enhanced training function with comprehensive visualization"""
    # Setup directories and visualization
    os.makedirs("training_runs", exist_ok=True)
    run_dir = get_next_run_dir("training_runs")
    os.makedirs(run_dir, exist_ok=True)
    
    # Initialize visualization system
    visualizer = setup_visualization(run_dir, "PPO_Humanoid_Walker", enable_live=enable_live_plots)
    ...
    visualizer.log_hyperparameters(hyperparams)
    ...
    print(f"TensorBoard: tensorboard --logdir {visualizer.tb_log_dir}")
    
    try:
        while timestep < total_timesteps:
            state = env.reset()
            agent.reset_episode()  # Reset RNN hidden states
            episode_reward = 0
            episode_length = 0
            episode_info_accumulator = defaultdict(list)
            episode_reward_breakdown = defaultdict(list)
            ...
            for step in range(env.max_episode_steps):
                ...
                # Accumulate info for episode logging
                for key, value in info.items():
                    if key != 'reward_breakdown' and isinstance(value, (int, float)):
                        episode_info_accumulator[key].append(value)
                
                # Accumulate reward breakdown
                if 'reward_breakdown' in info:
                    for reward_type, reward_value in info['reward_breakdown'].items():
                        episode_reward_breakdown[reward_type].append(reward_value)
                ...
                if len(agent.states) >= 2048:
                    ...
                    visualizer.log_training_metrics(timestep, losses, agent.learning_stats)
                    
                    # Log policy metrics
                    policy_stats = agent.get_policy_stats()
                    visualizer.log_policy_metrics(timestep, policy_stats)
            ...
            episode_info_avg = {}
            for key, values in episode_info_accumulator.items():
                if values:
                    episode_info_avg[key] = np.mean(values)
            
            # Process reward breakdown
            episode_reward_breakdown_avg = {}
            for reward_type, values in episode_reward_breakdown.items():
                if values:
                    episode_reward_breakdown_avg[reward_type] = np.mean(values)
            ...
            visualizer.log_episode_metrics(
                ...
                episode_info=episode_info_avg,
                reward_breakdown=episode_reward_breakdown_avg
            )
            
            # Log environment metrics
            env_stats = {
                'episodes_completed': episode + 1,
                'avg_episode_reward_last_100': np.mean(list(episode_rewards)[-100:]) if episode_rewards else episode_reward,
                'avg_episode_length_last_100': np.mean(list(episode_lengths)[-100:]) if episode_lengths else episode_length
            }
            visualizer.log_environment_metrics(timestep, env_stats)
            ...
            if episode % 10 == 0:
                ...
                print(f"  Final Height: {episode_info_avg.get('height', 'N/A')}")
                if episode_reward_breakdown_avg:
                    print("  Reward Breakdown:")
                    for reward_type, value in episode_reward_breakdown_avg.items():
            ...
            if timestep % save_interval == 0:
                ...
                visualizer.create_training_summary_plots(
                    save_path=f"{run_dir}/training_summary_{timestep}.png"
                )
        ...
        print("Creating final training analysis...")
        visualizer.create_training_summary_plots()
        ...
        agent.save(f"{run_dir}/ppo_humanoid_interrupted.pth")
        visualizer.close()
    
    except Exception as e:
        print(f"Training error: {e}")
        agent.save(f"{run_dir}/ppo_humanoid_error.pth")
        visualizer.close()
        raise
...
def test_trained_model(xml_path, model_path, episodes=5, run_mode="human"):
    ...
    # Setup visualization for testing
    test_dir = os.path.join(os.path.dirname(model_path), "test_results")
    os.makedirs(test_dir, exist_ok=True)
    
    visualizer = RLTrainingVisualizer(test_dir, "PPO_Testing")
    ...
    for episode in range(episodes):
        ...
        visualizer.log_episode_metrics(
            episode=episode,
            timestep=episode_length * (episode + 1),
            episode_reward=episode_reward,
            episode_length=episode_length,
            episode_info=episode_info_avg,
            reward_breakdown=episode_reward_breakdown_avg
    ...
    print(f"  Best Episode: {np.max(test_rewards):.2f}")
    print(f"  Worst Episode: {np.min(test_rewards):.2f}")
    
    # Create test plots
    visualizer.create_training_summary_plots(save_path=f"{test_dir}/test_summary.png")
    visualizer.close()
...
def test_sim():
    """Test simulation with basic visualization"""
...
def compare_training_runs(run_dirs, run_names=None):
    """Compare multiple training runs using visualization system"""
    from rl_visualization import create_comparison_plots
    
    if run_names is None:
        run_names = [f"Run {i+1}" for i in range(len(run_dirs))]
    
    print("Creating comparison plots...")
    fig = create_comparison_plots(run_dirs, run_names)
    
    # Also create detailed analysis for each run
    for run_dir, run_name in zip(run_dirs, run_names):
        print(f"\nAnalyzing {run_name} ({run_dir}):")
        try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a bipedal humanoid using MuJoCo and PPO with comprehensive visualization")
    parser.add_argument("--task", type=str, default="train", 
                       help="Task: 'train', 'test', 'analyze', or 'compare'")
    parser.add_argument("--model_sim", type=str, default="g1/scene_23dof.xml", 
                       help="Path to the MuJoCo XML model file")
    parser.add_argument("--max_steps", type=int, default=100000, 
                       help="Total timesteps for training")
    parser.add_argument("--run_mode", type=str, default=None, 
                       help="Render mode: 'view' for visual or None for headless")
    parser.add_argument("--load_checkpoint", type=str, default="training_runs/run_1/ppo_humanoid_final.pth", 
                       help="Path to checkpoint .pth file")
    parser.add_argument("--test_episodes", type=int, default=5, 
                       help="Number of episodes for testing")
    parser.add_argument("--enable_live_plots", action="store_true", 
                       help="Enable live plotting during training")
    parser.add_argument("--save_interval", type=int, default=10000, 
                       help="Save model every N timesteps")
    parser.add_argument("--run_dirs", nargs="+", 
                       help="List of run directories for comparison")
    parser.add_argument("--run_names", nargs="+", 
                       help="Names for runs in comparison")

    args = parser.parse_args()
    model_path = os.path.join(SCRIPT_DIR, args.model_sim)
    
    print("Bipedal Humanoid Training with Advanced Visualization")
    print("="*60)

    if args.task == "train":
        print("Starting training with visualization...")
        agent, rewards = train_humanoid_walker(
            xml_path=model_path, 
            total_timesteps=args.max_steps,
            run_mode=args.run_mode,
            enable_live_plots=args.enable_live_plots,
        ...
        test_rewards, test_lengths = test_trained_model(
            xml_path=model_path, 
        ...
        if os.path.exists(args.load_checkpoint):
            run_dir = os.path.dirname(args.load_checkpoint)
            analyze_training_run(run_dir)
        else:
        ...
        if args.run_dirs:
            ...
            compare_training_runs(args.run_dirs, args.run_names)
        else:
            print("Please provide run directories with --run_dirs")

    elif args.task == "sim_test":
        print("Testing simulation...")
        test_sim()

    else:
        print(f"Unknown task: {args.task}")
        print("Available tasks: train, test, analyze, compare, sim_test")

    print("\nVisualization Tips:")
    print("1. Use 'tensorboard --logdir training_runs/run_X/tensorboard_logs' for real-time monitoring")
    print("2. Check the generated PNG files for static analysis")
    print("3. Use --enable_live_plots for live dashboard during training")
    print("4. Compare runs using: --task compare --run_dirs run_1 run_2 run_3")